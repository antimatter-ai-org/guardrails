...
from app.model_assets import apply_model_env, resolve_token_classifier_model_source
...
def run() -> None:
    ...
    enable_nemotron = _env("GR_ENABLE_NEMOTRON", "true").strip().lower() in {"1", "true", "yes", "on"}
    ...
    if not enable_nemotron:
        raise RuntimeError("no PyTriton models enabled (set GR_ENABLE_NEMOTRON=true)")
