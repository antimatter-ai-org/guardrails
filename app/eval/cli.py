...
from app.eval.config import EvalRegistry, load_eval_registry
from app.eval.datasets.hf_span_dataset import build_samples_from_hf_split, load_hf_split, scan_hf_span_dataset
from app.eval.datasets.splits import SamplerSpec, SubsetSpec, load_or_create_indices
from app.eval.reporting.schema import REPORT_VERSION
from app.eval.reporting.write_outputs import write_report_files
from app.eval.tasks.leakage import MaskLeakageInputs, run_mask_leakage
from app.eval.tasks.policy_action import PolicyActionInputs, run_policy_action
...
def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Guardrails evaluation runner.")
    ...
    p.add_argument("--action-policies", default="external", help="Comma-separated policy ids for policy_action.")
...
def main() -> int:
    ...
    run_id = f"eval_{suite_name}_{split}_{started_at.strftime('%Y%m%dT%H%M%SZ')}_{run_tag}"
    ...
    try:
        ...
        if "policy_action" in tasks:
            ...
            for policy_name in action_policy_names:
                ...
                from app.eval.reporting.schema import binary_counts_payload
